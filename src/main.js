...
import skyBoxMaterialsPromise from "./materials/skyBoxMaterialsPromise";
...
async function initScene (setup = (scene, camera, controllers, players) => {}) {
    ...
    function createSkyBox(size, color, skyTexture) {
...
`
        ...
        skyBoxMaterialsPromise
            .then((materialsArray) => {

                for (let m of materialsArray) {
                    console.log("SkyBox material:", m);
                    m.side = THREE.DoubleSide;

                    // const texture = m.map;
                    // const textureRepeatScale = 1;
                    // texture.repeat.set(textureRepeatScale, textureRepeatScale);
                    // texture.repeat.x = textureRepeatScale;
                    // texture.repeat.y = textureRepeatScale;
                    // texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
...
`
    ...
    let skyBoxMaterialUpdated = false;

    // const skyInsideMesh  = createSphere(50.0, mapColors.get("orangeDark"), canvasTexture);
    const skyInsideMesh  = createSkyBox(100.0, mapColors.get("orangeDark"), canvasTexture);
    ...
    function renderWorld() {
        ...
        skyBoxMaterialsPromise
            .then((materialsArray) => {
                ...
                setTimeout(() => {
                    if (!skyBoxMaterialUpdated) {
                        skyBoxMaterialUpdated = true;

                        for (let m of skyInsideMesh.material) {
                            console.log("SkyBox updated:", m);
                        }
                    }
                }, 0);
            });

        if (!!skyBoxMaterialUpdated) for (let m of skyInsideMesh.material) {
            m.clippingPlanes = [
                clippingPlaneOutside,
                clippingTopPlane,
                clippingBottomPlane,
        ...
        } else if (skyInsideMesh.material.hasOwnProperty("clippingPlanes")) {
